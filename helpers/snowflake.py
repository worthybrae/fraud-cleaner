    ...
    try:
        query = f"""
            ...
            where
                query_id = '{query_id}'
        """
        cursor.execute(f"use warehouse {st.secrets('snowflake_warehouse_sm')}")
...
def fetch_results_from_query_id(conn, query_id):
    ...
    try:
        ...
        cursor.execute(f"use warehouse {st.secrets('snowflake_warehouse_lg')}")
...
def query_snowflake(query, user, password, account, wh, database, schema, delay=15, warehouse='snowflake_warehouse_lg', execute_async=True):
    ...
    try:
        ...
        warehouse_selection = f"use warehouse {st.secrets(warehouse)};"
